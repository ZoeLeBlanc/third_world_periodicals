...
# full_corpus_df = full_corpus_df.loc[full_corpus_df.date < '1962-January-1']

full_corpus_df.cleaned_spacy_text = full_corpus_df.cleaned_spacy_text.fillna('')

df = pd.read_csv('../data/congo_full_corpus_training2.csv')
filename = '../data/chap4_congo_classifier2'

df['class_numb'] = 0
df.class_numb[df.classify =='congo'] = '0'
df.class_numb[df.classify =='no_congo'] = '1'
df.class_numb = df.class_numb.astype(int)
df.cleaned_spacy_text = df.cleaned_spacy_text.fillna('')
...
train_model(df, filename, full_corpus_df)
